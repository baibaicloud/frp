...
func (pxy *TcpProxy) Run() (remoteAddr string, err error) {
	...
	if pxy.cfg.Group != "" {
		...
		resp, err2 := http.Get(fmt.Sprintf("%s?proxyname=%s&remoteport=%d", pxy.serverCfg.PortCheckUrl, pxy.cfg.ProxyName, pxy.cfg.RemotePort))
		if err2 != nil {
			return
		}
		...
		if string(body) != "ok" {
			err = fmt.Errorf("proxy port check error" + pxy.cfg.ProxyName)
			return
		}
